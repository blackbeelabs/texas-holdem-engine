...
class WinningHandSelector:
    ...
    def validate_hands(
        ...
        for card in all_cards:
            ...
            if value not in HandEvaluator.CARD_VALUES:
                raise ValueError(
                    f"Invalid card value: {value}. Must be one of: {list(HandEvaluator.CARD_VALUES.keys())}"
                )

            if suit not in HandEvaluator.VALID_SUITS:
                raise ValueError(
                    f"Invalid card suit: {suit}. Must be one of: {HandEvaluator.VALID_SUITS}"
                )
